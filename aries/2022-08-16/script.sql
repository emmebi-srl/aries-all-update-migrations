DELETE FROM environment_variables WHERE environment_variables.var_key = 'ENABLE_CUSTOMER_DASHBOARD';
...
BEGIN
	...
	IF is_emmebi THEN
		insert into environment_variables VALUES ('ENABLE_CUSTOMER_DASHBOARD', '1', NOW());
	ELSE
		insert into environment_variables VALUES ('ENABLE_CUSTOMER_DASHBOARD', '0', NOW());
	END IF;
...
DROP TABLE IF EXISTS rapporto_totali;
CREATE TABLE `rapporto_totali` (
	...
	PRIMARY KEY (`id`),
	UNIQUE INDEX `rapporto_totali_id_anno` (`Id_rapporto`, `anno`),
	CONSTRAINT `fk_rapporto_rapporto` FOREIGN KEY (`id_rapporto`, `anno`) REFERENCES `rapporto` (`Id_rapporto`, `anno`) ON UPDATE CASCADE ON DELETE CASCADE
)
COMMENT='Storico dei totali rapporto'
...
DROP TABLE IF EXISTS resoconto_totali;
CREATE TABLE `resoconto_totali` (
	`id` INT(11) NOT NULL AUTO_INCREMENT,
	...
	`prezzo_totale` DECIMAL(10,2) NOT NULL,
	PRIMARY KEY (`id`),
	UNIQUE INDEX `resoconto_totali_id_anno` (`id_resoconto`, `anno`),
	CONSTRAINT `fk_resoconto_totali_resoconto` FOREIGN KEY (`id_resoconto`, `anno`) REFERENCES `resoconto` (`Id_resoconto`, `anno`) ON UPDATE CASCADE ON DELETE CASCADE
)
COMMENT='Storico dei totali resoconto'
COLLATE='latin1_swedish_ci'
ENGINE=InnoDB
;
