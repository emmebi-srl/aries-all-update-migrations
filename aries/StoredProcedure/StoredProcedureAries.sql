...
CREATE  PROCEDURE `sp_ariesEmailInsert`(
	IN `sender_id` INT(11),
	IN `sender_address` VARCHAR(150),
	IN `reply_to` VARCHAR(150),
...
BEGIN
	...
	IF Result >= 0 THEN
		...
		SET 		
			...
			Letto = is_read,
			Risposto = got_reply,
			Conferma_lettura = read_confirmation,
...
BEGIN
	...
	SELECT 
		...
		`estratto_conto_reply_to_email`,
		`estratto_conto_indirizzo_mittente`,
		`utente_mod`,
		`data_mod`
